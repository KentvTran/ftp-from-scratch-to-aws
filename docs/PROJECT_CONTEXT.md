...
- **Control Port:** `2121` (non-privileged; configurable via `FTP_PORT`)
- **Data Port Range:** `20000` - `21000`
- **Connection Flow:**
    1.  Client connects to Server on Control Port (`2121`).
...
- **Commands (Client -> Server):**
    - `GET <filename>`
    - `PUT <filename> SIZE <filesize>`
    - `LS`
    - `EXIT`
- **Response Codes (Server -> Client):**
    - `200 OK [data]`: Command valid, proceeding. (e.g., `200 OK 20001`)
    - `226 Listing complete / 226 Transfer complete / 226 File stored`: Action finished successfully (depends on command).
...
- **COST CONTROL:** Kent manually starts and stops the `t3.micro` instance for specific "Testing Windows" to stay under a $5 budget.
- **Security Group:**
    - Port `22` (SSH): Open to `0.0.0.0/0`
    - Port `2121` (Control): Open to `0.0.0.0/0`
